...
public interface ClockInput {
	...
	interface UserInput {
		/**
		 * Returns CHOICE_SET_TIME if the user entered a new clock time,
		 * CHOICE_SET_ALARM if the user entered a new alarm time, or CHOICE_TOGGLE_ALARM
		 * if the user pressed both simultaneously (to turn the alarm on/off).
		 */
		int getChoice();
		 ...
		 * 
		 * If getChoice() returns CHOICE_SET_TIME, these return the time the user
		 * entered. If getChoice() returns CHOICE_SET_ALARM, these return the alarm time
		 * the user entered. If getChoice() returns CHOICE_TOGGLE_ALARM, these return an
		 * invalid value.
		 */
		int getHours();
